...
def nationality_search(countries, cur, conn):
    ...
    r = f'''SELECT name, position_id, nationality FROM Players WHERE nationality IN ({country_placeholders})'''

    cur.execute(r, countries)
...
def birthyear_nationality_search(age, country, cur, conn):
    r = '''SELECT name, nationality, birthyear 
            FROM Players 
            WHERE nationality = ? AND birthyear < ? '''
    cur.execute(r, (country, 2023 - age))
